...
export const CombinationsTable = ({ initialRows }: CombinationsTableProps) => {
  ...
  // Sync rows if server component refetches data (e.g. via router.refresh)
  useEffect(() => {
    setRows(initialRows);
  }, [initialRows]);
  ...
  return (
    <div className="bg-brand-surface border border-brand-border/60 rounded-xl overflow-hidden shadow-sm">
      <div className="px-6 py-5 border-b border-brand-border/60 bg-surface-neutral/30">
        <h2 className="font-heading font-bold text-text-primary text-lg tracking-tight">{d.tableTitle}</h2>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-brand-border/60 bg-surface-neutral/50">
              <th className="text-left px-6 py-4 font-semibold text-text-secondary tracking-wide uppercase text-xs">{d.table.region}</th>
              <th className="text-left px-6 py-4 font-semibold text-text-secondary tracking-wide uppercase text-xs">{d.table.pest}</th>
              <th className="text-left px-6 py-4 font-semibold text-text-secondary tracking-wide uppercase text-xs">{d.table.status}</th>
              <th className="text-right px-6 py-4 font-semibold text-text-secondary tracking-wide uppercase text-xs">{d.table.actions}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
