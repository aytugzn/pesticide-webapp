...
export const GoogleStats = ({
  ...
  const displayReviewCount =
    stats?.reviewCount || DICTIONARY.home.googleStats.reviewCount;
    ...
    <div className="flex items-center w-full pt-6 border-t border-brand-border/50">
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col justify-center gap-1.5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center items-start gap-0.5 sm:gap-1.5 mt-0.5">
            <div className="flex items-center gap-1.5">
              <span className="text-sm md:text-base font-black text-text-primary leading-none">
                {displayRating}
              </span>
              <div
              ...
              >
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className="w-3.5 h-3.5 md:w-4 md:h-4 fill-google-yellow text-google-yellow"
                    aria-hidden="true"
                  />
                ))}
              </div>
