...
const MenuBar = ({ editor }: { editor: Editor | null }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 p-2 border-b border-brand-border bg-brand-surface-light rounded-t-brand-md">
      <Button
        ...
        className={`min-h-9 min-w-9 p-2 rounded-brand-sm transition-colors ${
      ...
      <div className="w-px h-6 flex-none bg-brand-border mx-0.5" />
      ...
      <Button
        ...
        className={`min-h-9 min-w-9 p-2 rounded-brand-sm transition-colors font-bold ${
      ...
      <Button
        ...
        className="min-h-9 min-w-9 p-2 rounded-brand-sm text-text-secondary hover:bg-brand-surface-muted transition-colors disabled:opacity-50"
...
export const RichTextEditor = ({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-sm sm:prose-base dark:prose-invert focus:outline-none max-w-none min-w-0 break-words p-3 sm:p-4 min-h-56 sm:min-h-72 text-text-primary",
      },
    },
    onUpdate: ({ editor }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Rich text editor sync failed");
    }
  }, [value, editor]);
  ...
  return (
    <div
      className={cn(
        `min-w-0 border border-brand-border rounded-brand-md overflow-hidden bg-background focus-within:ring-2 focus-within:ring-brand-primary/20 focus-within:border-brand-primary transition-colors ${className}`,
      )}
    >
